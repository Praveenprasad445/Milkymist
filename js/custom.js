...
function user_register(){
	...
	var phonc=new RegExp(/^[1-9]\d{9}$/);
	var namec=new RegExp(/^[A-Za-z]{3,}$/);
	var is_error='';
	if(name==""){
	...
	if(!passw.test(password)){
		jQuery('#password_error').html('Please enter a valid password');
		is_error='yes';
	}
	if(is_error==''){
		jQuery.ajax({
...
function user_login(){
	...
	if(!passw.test(password)){
		jQuery('#login_password_error').html('Please enter a valid password');
		is_error='yes';
	}
	if(is_error==''){
		jQuery.ajax({
			...
			success:function(result){
				if(result=='wrong'){
					jQuery('.login_msg p').html('Please enter valid login details or you are not approved by Admin ');
				}
				if(result=='valid'){
